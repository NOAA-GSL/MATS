...
def mysql_connect_and_check_tables():
    ...
    if cursor.rowcount == 0:
        print("mats_metadata db does not exist. Run MEsurface.py to initialize it.")
        sys.exit(1)
    ...
    cnx.commit()
    if cursor.rowcount == 0:
        print("surface metadata table does not exist. Run MEsurface.py to initialize it.")
        sys.exit(1)
    ...
    if cursor.rowcount == 0:
        print("surface groups table does not exist. Run MEsurface.py to initialize it.")
        sys.exit(1)
...
def update_model_in_metadata_table(cnx, cursor, mvdb, model, raw_metadata):
    ...
    cursor.execute("select * from surface_mats_metadata where db = '" + mvdb + "' and model = '" + model + "';")
...
if __name__ == '__main__':
    ...
    msg = 'SURFACE MATS FOR MET UPDATE METADATA START: ' + utc_now
    print(msg)
    main()
    utc_now = str(datetime.now())
    msg = 'SURFACE MATS FOR MET UPDATE METADATA END: ' + utc_now
    print(msg)
    sys.exit(0)
