...
def mysql_prep_tables():
    ...
    print("Checking for air quality metadata tables")
    cursor.execute('show tables like "airquality_mats_metadata_dev";')
    cnx.commit()
    if cursor.rowcount == 0:
        ...
        cnx.commit()
    cursor.execute('show tables like "airquality_mats_metadata";')
    cnx.commit()
    if cursor.rowcount == 0:
    ...
    cursor.execute("delete from airquality_mats_metadata_dev;")
    cnx.commit()

    # see if the metadata group tables already exist
    cursor.execute('show tables like "airquality_database_groups_dev";')
    if cursor.rowcount == 0:
        ...
        cursor.execute(create_table_query)
        cnx.commit()
    cursor.execute('show tables like "airquality_database_groups";')
    if cursor.rowcount == 0:
    ...
    cursor.execute("delete from airquality_database_groups_dev;")
    cnx.commit()
...
def deploy_dev_table_and_close_cnx(cnx, cursor):
    ...
    cursor.execute("use mats_metadata;")
    cnx.commit()
    cursor.execute("delete from airquality_mats_metadata;")
    cnx.commit()
    cursor.execute("insert into airquality_mats_metadata select * from airquality_mats_metadata_dev;")
    cnx.commit()
    cursor.execute("delete from airquality_database_groups;")
    cnx.commit()
    cursor.execute("insert into airquality_database_groups select * from airquality_database_groups_dev;")
    cnx.commit()
...
def build_stats_object(cnx, cursor):
    ...
    for mvdb in mvdbs:
        ...
        for line in cursor:
            ...
            print("Getting thresholds for model " + model)
            cursor2.execute(get_trshs)
            cnx2.commit()
            for line2 in cursor2:
                trsh = line2.values()[0]
                per_mvdb[mvdb][model]['trshs'].append(trsh)
            per_mvdb[mvdb][model]['trshs'].sort(key=strip_trsh)
