#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in vis_1min and vis_1min_sums
...
def update_rpm_record(cnx, cursor, table_name, display_text, regions, fcst_lens, trshs, truths, display_category, display_order, mindate, maxdate, numrecs):
    ...
    if len(regions) > int(0) and len(fcst_lens) > int(0):
        ...
        if record_id == 0:
            ...
            update_rpm_rec = "UPDATE regions_per_model_mats_all_categories_build SET regions = %s, fcst_lens = %s, trsh = %s, truth = %s, display_category = %s, display_order = %s, mindate = %s, maxdate = %s, numrecs = %s, updated = %s WHERE id = %s"
            qd.append(str(regions))
            qd.append(str(fcst_lens))
            qd.append(str(trshs))
            qd.append(str(truths))
...
def regions_per_model_mats_all_categories(mode):
    ...
    for row in cursor:
        ...
        if tablename not in skiptables:
            ...
            show_tables = "show tables like '" + str(model) + "_%';"
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            get_fcst_lens = ("SELECT DISTINCT fcst_len,fcst_min FROM " + tablename + ";")
            cursor.execute(get_fcst_lens)
            per_table[tablename]['fcst_lens'] = []
            this_fcst_lens = []
            for row in cursor:
                val = int(row['fcst_len'] * 60) + int(row['fcst_min'])
            ...
            # get truths from this table
            get_truths = ("SELECT DISTINCT truth FROM " + tablename + ";")
            cursor.execute(get_truths)
            per_table[tablename]['truths'] = []
            this_truths = []
            for row in cursor:
                val = row.values()[0]
                this_truths.append(val)
            per_table[tablename]['truths'] = this_truths
            # print(tablename + " this_truths: " + str(per_table[tablename]['this_truths']) )
            ...
            replace_tablestats_rec = "REPLACE INTO TABLESTATS_build (tablename, mindate, maxdate, model, region, fcst_lens, trsh, truth, numrecs) values( %s, %s, %s, %s, %s, %s, %s, %s, %s )"
            qd = []
            ...
            qd.append(str(per_table[tablename]['truths']))
    ...
    for model in all_data_sources:
        ...
        # get truths for all tables pertaining to this model
        get_these_truths = "select distinct(truth) from " + db + ".TABLESTATS_build where tablename like '" + model + "%' and truth != '[]' and model = '" + model + "' and numrecs > 0 order by length(truth) desc;"
        cursor.execute(get_these_truths)
        these_truths = []
        for row in cursor:
            val_array = ast.literal_eval(row.values()[0])
            for val in val_array:
                if val not in these_truths:
                    these_truths.append(val)
        these_truths.sort()
        # print( "these_trshs:\n" + str(these_trshs) )
        ...
        if len(these_regions) > 0 and len(these_fcst_lens) > 0 and len(these_trshs) > 0 and len(these_truths) > 0:
            update_rpm_record(cnx, cursor, model, display_text, these_regions, these_fcst_lens, these_trshs, these_truths, cat, do, catstats['mindate'], catstats['maxdate'], catstats['numrecs'])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] == 'deploy':
            ...
            msg = 'VISIBILITY_1 MATS METADATA START: ' + utcnow
            print(msg)
            regions_per_model_mats_all_categories('deploy')
            utcnow = str(datetime.now())
            msg = 'VISIBILITY_1 MATS METADATA END: ' + utcnow
            print(msg)
