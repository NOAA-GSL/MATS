...
def regions_per_model_mats_all_categories(mode):
    ...
    for model in all_data_sources:
        ...
        get_these_truths = "select distinct(truth) from " + db + ".TABLESTATS_build where tablename like '" + \
            model + "%' and truth != '[]' and model = '" + model + \
            "' and numrecs > 0 order by length(truth) desc;"
        ...
        if len(these_regions) > 0 and len(these_fcst_lens) > 0 and len(these_trshs) > 0 and len(these_truths) > 0:
            update_rpm_record(cnx, cursor, model, display_text, these_regions, these_fcst_lens, these_trshs,
                              these_truths, cat, do, catstats['mindate'], catstats['maxdate'], catstats['numrecs'])
