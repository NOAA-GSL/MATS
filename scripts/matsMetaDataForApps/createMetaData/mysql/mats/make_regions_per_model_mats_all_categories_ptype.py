#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in ptype2
...
def update_rpm_record(cnx, cursor, table_name, display_text, regions, fcst_lens, scales, display_category, display_order, mindate, maxdate, numrecs):
    ...
    if len(regions) > int(0) and len(fcst_lens) > int(0):
        ...
        if record_id == 0:
            insert_rpm_rec = "INSERT INTO regions_per_model_mats_all_categories_build (model, display_text, regions, fcst_lens, scale, display_category, display_order, id, mindate, maxdate, numrecs, updated) values( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )"
            ...
            update_rpm_rec = "UPDATE regions_per_model_mats_all_categories_build SET regions = %s, fcst_lens = %s, scale = %s, display_category = %s, display_order = %s, mindate = %s, maxdate = %s, numrecs = %s, updated = %s WHERE id = %s"
            qd.append(str(regions))
            qd.append(str(fcst_lens))
            qd.append(str(scales))
            qd.append(display_category)
...
def regions_per_model_mats_all_categories(mode):
    ...
    skiptables = "ptype_descriptions regions_per_model_mats_all_categories regions_per_model_mats_all_categories_build scale_descriptions t1"

    # get an array of all relevant data sources in this db
    all_data_sources = []
    per_table = {}

    show_tables = "show tables like '%_freq_%';"
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            if tablename.startswith('obs_freq'):
                this_fcst_lens = [0]
            else:
            ...
            get_tablestats = "SELECT min(valid_secs) AS mindate, max(valid_secs) AS maxdate, count(valid_secs) AS numrecs FROM " + tablename + ";"
    ...
    for model in all_data_sources:
        ...
        get_these_scales = "select distinct(scle) from " + db + ".TABLESTATS_build where tablename like '" + model + "%' and scle != '[]' and model = '" + model + "' and numrecs > 0 order by length(scle) desc;"
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] == 'deploy':
            ...
            msg = 'PTYPE2 MATS METADATA START: ' + utcnow
            print(msg)
            regions_per_model_mats_all_categories('deploy')
            utcnow = str(datetime.now())
            msg = 'PTYPE2 MATS METADATA END: ' + utcnow
            print(msg)
