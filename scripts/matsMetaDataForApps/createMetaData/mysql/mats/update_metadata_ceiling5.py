#!/usr/bin/env python
#
# Updates the regions_per_model_mats_all_categories table for all models in ceiling_5min_sums
...
def update_rpm_record(cnx, cursor, table_name, display_text, regions, fcstlens, trshs, display_category, display_order,
    ...
    if len(regions) > int(0) and len(fcstlens) > int(0):
        ...
        if record_id == 0:
            ...
            cnx.commit()
            insert_rpm_rec = "INSERT INTO regions_per_model_mats_all_categories (model, display_text, regions, fcst_lens, trsh, display_category, display_order, id, mindate, maxdate, numrecs, updated) values( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )"
            cursor.execute(insert_rpm_rec, qd)
            cnx.commit()
        else:
            ...
            cnx.commit()
            update_rpm_rec = "UPDATE regions_per_model_mats_all_categories SET regions = %s, fcst_lens = %s, trsh = %s, display_category = %s, display_order = %s, mindate = %s, maxdate = %s, numrecs = %s, updated = %s WHERE id = %s"
            cursor.execute(update_rpm_rec, qd)
            cnx.commit()
...
def reprocess_specific_metadata(models_to_reprocess):
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_lens']) > 0 and len(per_model[model]['trshs']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['region'], per_model[model]['fcst_lens'], per_model[model]['trshs'], per_model[model]['display_category'], per_model[model]['display_order'], per_model[model]['mindate'], per_model[model]['maxdate'], per_model[model]['numrecs'])
