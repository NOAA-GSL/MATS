#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in precip_mesonets2 and precip_mesonets2_sums
...
def regions_per_model_mats_all_categories(mode):
    ...
    skiptables = "all_display_categories all_display_categories_build all_display_categories_dev regions_per_model_mats_all_categories regions_per_model_mats_all_categories_build regions_per_model_mats_all_categories_dev threshold_descriptions template TABLESTATS TABLESTATS_build TABLESTATS_dev cum_pcp2 hourly_pcp2 pcp_stations3 region_descriptions locations2 locations2_bak pcp_stations2 cum_pcp hourly_pcp pc_stations_bak pcp_stations t1"
        ...
        if tablename not in skiptables:
            ...
            for row2 in cursor2:
                ...
                if region in valid_regions.keys():
                    ...
                    per_table[sums_table]['source'] = source
                    # print("Table is: " + t + ", Model is: " + model + ", Region is: " + region + ", Source is: " + source)
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            get_trshs = ("SELECT DISTINCT thresh FROM " + tablename + ";")
            ...
            replace_tablestats_rec = "REPLACE INTO TABLESTATS_build (tablename, mindate, maxdate, model, region, sources, fcst_lens, thresh, numrecs) values( %s, %s, %s, %s, %s, %s, %s, %s, %s )"
            qd = []
    ...
    for model in all_data_sources:
        ...
        if len(these_regions) > 0 and len(these_fcst_lens) > 0 and len(these_trshs) > 0 and len(these_sources) > 0:
            update_rpm_record(cnx, cursor, model, display_text, these_regions, these_sources, these_fcst_lens, these_trshs, cat, do, catstats['mindate'], catstats['maxdate'], catstats['numrecs'])
