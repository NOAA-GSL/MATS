#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in precip
...
def regions_per_model_mats_all_categories(mode):
    ...
    skiptables = "all_display_categories all_display_categories_build all_display_categories_dev regions_per_model_mats_all_categories regions_per_model_mats_all_categories_dev regions_per_model_mats_all_categories_build template TABLESTATS_build fcst_type_descriptions scale_descriptions threshold_descriptions"
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            get_fcst_types = ("SELECT DISTINCT num_fcsts FROM " + tablename + ";")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] == 'deploy':
            ...
            msg = 'PRECIP 24 HR MATS METADATA START: ' + utcnow
            print(msg)
            regions_per_model_mats_all_categories('deploy')
            utcnow = str(datetime.now())
            msg = 'PRECIP 24 HR MATS METADATA END: ' + utcnow
            print(msg)
