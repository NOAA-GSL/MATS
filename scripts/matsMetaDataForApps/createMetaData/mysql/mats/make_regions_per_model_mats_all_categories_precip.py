...
def regions_per_model_mats_all_categories(mode):
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            get_fcst_types = (
                "SELECT DISTINCT num_fcsts FROM " + tablename + ";")
    ...
    for model in all_data_sources:
        ...
        get_these_fcst_types = "select distinct(fcst_types) as fcst_types from " + db + ".TABLESTATS_build where tablename like '" + \
            model + "%' and fcst_types != '[]' and model = '" + model + \
            "' and numrecs > 0 order by length(fcst_types) desc;"
        ...
        if len(these_regions) > 0 and len(these_fcst_types) > 0 and len(these_trshs) > 0:
            update_rpm_record(cnx, cursor, model, display_text, these_regions, these_fcst_types, these_trshs,
