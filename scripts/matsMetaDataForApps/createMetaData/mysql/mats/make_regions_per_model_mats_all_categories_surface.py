#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in surface_sums2 that contain '_metar_v*_' in their name
...
def regions_per_model_mats_all_categories(mode):
    ...
    for metar_type in metar_types:
        ...
        for row in cursor:
            ...
            if tablename not in skiptables:
                ...
                model = re.sub("_" + metar_type + "\d[A-Za-z]?_.*", "", tablename)
                if model not in all_data_sources:
                    all_data_sources.append(model)
                per_table[tablename] = {}
                per_table[tablename]['model'] = model
                temp = "^" + model + "_" + metar_type + "\d[A-Za-z]?_"
                region = re.sub(temp, "", tablename)
                per_table[tablename]['region'] = region
                temp1 = re.sub("^" + model + "_", "", tablename)
                metar_str = re.sub("_" + region + "$", "", temp1)
                per_table[tablename]['metar_string'] = metar_str
                # print("model is " + model + ", region is " + region + ", metar_string is " + metar_str)
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            replace_tablestats_rec = "REPLACE INTO TABLESTATS_build (tablename, metar_string, mindate, maxdate, model, region, fcst_lens, numrecs) values( %s, %s, %s, %s, %s, %s, %s, %s )"
            qd = []
            qd.append(str(tablename))
            qd.append(str(per_table[tablename]['metar_string']))
    ...
    get_metar_descriptions = "select metar_string,description from metar_string_descriptions;"
    cursor4.execute(get_metar_descriptions)

    metar_source_map = {}
    for row in cursor4:
        metar_string = str(row['metar_string'])
        description = str(row['description'])
        metar_source_map[metar_string] = description
    ...
    for model in all_data_sources:
        ...
        # get metar_strings for all tables pertaining to this model
        get_these_metar_strings = "select distinct(metar_string) as metar_string from " + db + ".TABLESTATS_build where tablename like '" + model + "%' and model = '" + model + "' and numrecs > 0;"
        cursor.execute(get_these_metar_strings)
        these_metar_strings = []
        these_sources = []
        for row in cursor:
            val = str(row.values()[0])
        ...
        if len(these_regions) > 0 and len(these_fcst_lens) > 0 and len(these_sources) > 0:
            update_rpm_record(cnx, cursor, model, display_text, these_sources, these_metar_strings, these_regions, these_fcst_lens, cat, do, catstats['mindate'], catstats['maxdate'], catstats['numrecs'])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] == 'deploy':
            ...
            msg = 'SURFACE_SUMS2 MATS METADATA START: ' + utcnow
            print(msg)
            regions_per_model_mats_all_categories('deploy')
            utcnow = str(datetime.now())
            msg = 'SURFACE_SUMS2 MATS METADATA END: ' + utcnow
            print(msg)
