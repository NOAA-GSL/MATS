#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in vil
...
def regions_per_model_mats_all_categories(mode):
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            for row in cursor:
                val = row.values()[0]
                this_trshs.append(float(val))
            this_trshs.sort(key=float)
