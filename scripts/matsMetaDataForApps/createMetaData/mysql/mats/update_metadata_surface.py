#!/usr/bin/env python
#
# Updates the regions_per_model_mats_all_categories table for all models in madis3 and surface_sums2 that contain '_metar_v*_' in their name
...
def update_rpm_record(cnx, cursor, table_name, display_text, sources, metar_string, regions, fcstlens, display_category,
...
def reprocess_specific_metadata(models_to_reprocess):
    ...
    for model in models_to_reprocess:
        ...
        for metar_type in metar_types:
            show_tables = ("show tables like '" + model + "_" + metar_type + "%';")
            cursor.execute(show_tables)
            for (line) in cursor:
                table_name = line.values()[0]
                table_string = table_name.encode('ascii', 'ignore')
                table_model = re.sub("_" + metar_type + "\d[A-Za-z]?_.*", "", table_string)
                if table_model == model:
                    ...
                    if int(stats['numrecs']) > 0:
                        ...
                        temp1 = re.sub("^" + model + "_", "", table_string)
                        metar_str = re.sub("_" + region + "$", "", temp1)
                        if metar_str not in per_model[model]['metar_strings']:
                            per_model[model]['metar_strings'].append(metar_str)
                            per_model[model]['sources'].append(metar_source_map[metar_str])
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_lens']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['sources'], per_model[model]['metar_strings'], per_model[model]['region'], per_model[model]['fcst_lens'], per_model[model]['display_category'], per_model[model]['display_order'], per_model[model]['mindate'], per_model[model]['maxdate'], per_model[model]['numrecs'])
