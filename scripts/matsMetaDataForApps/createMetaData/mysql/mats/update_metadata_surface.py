...
def reprocess_specific_metadata(models_to_reprocess):
    ...
    for model in models_to_reprocess:
        ...
        for metar_type in metar_types:
            ...
            for row in cursor:
                ...
                table_model = re.sub("_" + metar_type + "\d[A-Za-z]?_.*", "", tablename)
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_len']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['sources'], per_model[model]['metar_strings'], per_model[model]['region'], per_model[model]['fcst_len'], per_model[model]['display_category'], per_model[model]['display_order'], per_model[model]['mindate'], per_model[model]['maxdate'], per_model[model]['numrecs'])
