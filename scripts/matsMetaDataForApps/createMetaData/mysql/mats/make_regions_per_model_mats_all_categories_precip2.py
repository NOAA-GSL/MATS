#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in precip2
...
def update_rpm_record(cnx, cursor, table_name, display_text, regions, fcst_types, trshs, scales, display_category, display_order, mindate, maxdate, numrecs):
    ...
    if len(regions) > int(0) and len(fcst_types) > int(0):
        qd = []
        updated_utc = datetime.utcnow().strftime('%s')
        # if it's a new record (it should be) add it
        if record_id == 0:
            insert_rpm_rec = "INSERT INTO regions_per_model_mats_all_categories_build (model, display_text, regions, fcst_types, trsh, scle, display_category, display_order, id, mindate, maxdate, numrecs, updated) values( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )"
            qd.append(str(table_name))
            qd.append(str(display_text))
            qd.append(str(regions))
            qd.append(str(fcst_types))
            qd.append(str(trshs))
            ...
            update_rpm_rec = "UPDATE regions_per_model_mats_all_categories_build SET regions = %s, fcst_types = %s, trsh = %s, scle = %s, display_category = %s, display_order = %s, mindate = %s, maxdate = %s, numrecs = %s, updated = %s WHERE id = %s"
            qd.append(str(regions))
            qd.append(str(fcst_types))
            qd.append(str(trshs))
            qd.append(str(scales))
...
def regions_per_model_mats_all_categories(mode):
    ...
    skiptables = "all_display_categories all_display_categories_build all_display_categories_dev regions_per_model_mats_all_categories regions_per_model_mats_all_categories_dev regions_per_model_mats_all_categories_build template TABLESTATS_build RAP_RRret_protoRAPv3_winter_newdom_20km_NE fcst_type_descriptions scale_descriptions threshold_descriptions"
    ...
    if TScleaned:
        for tablename in per_table.keys():
            # get forecast types from this table
            get_fcst_types = ("SELECT DISTINCT accum_len FROM " + tablename + ";")
            cursor.execute(get_fcst_types)
            per_table[tablename]['fcst_types'] = []
            this_fcst_types = []
            for row in cursor:
                val = row.values()[0]
                this_fcst_types.append(int(val))
            this_fcst_types.sort(key=int)
            per_table[tablename]['fcst_types'] = this_fcst_types
            # print(tablename + " fcst_types: " + str(per_table[tablename]['fcst_types']) )
            ...
            replace_tablestats_rec = "REPLACE INTO TABLESTATS_build (tablename, mindate, maxdate, model, region, fcst_types, trsh, scle, numrecs) values( %s, %s, %s, %s, %s, %s, %s, %s, %s )"
            qd = []
            ...
            qd.append(str(per_table[tablename]['fcst_types']))
    ...
    try:
        ...
        cursor4.execute("set global connect_timeout=28800")
        cursor4.execute("set global wait_timeout=28800")
        cursor4.execute("set global interactive_timeout=28800")
    except MySQLdb.Error as e:
        print("Error: " + str(e))
        sys.exit(1)
    ...
    for model in all_data_sources:
        ...
        # get forecast types for all tables pertaining to this model
        get_these_fcst_types = "select distinct(fcst_types) as fcst_types from " + db + ".TABLESTATS_build where tablename like '" + model + "%' and fcst_types != '[]' and model = '" + model + "' and numrecs > 0 order by length(fcst_types) desc;"
        cursor.execute(get_these_fcst_types)
        these_fcst_types = []
        for row in cursor:
            val_array = ast.literal_eval(row.values()[0])
            for val in val_array:
                if val not in these_fcst_types:
                    these_fcst_types.append(val)
        these_fcst_types.sort(key=int)
        ...
        get_these_scales = "select distinct(scle) from " + db + ".TABLESTATS_build where tablename like '" + model + "%' and scle != '[]' and model = '" + model + "' and numrecs > 0;"
        ...
        if len(these_regions) > 0 and len(these_fcst_types) > 0 and len(these_trshs) > 0:
            update_rpm_record(cnx, cursor, model, display_text, these_regions, these_fcst_types, these_trshs, these_scales, cat, do, catstats['mindate'], catstats['maxdate'], catstats['numrecs'])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] == 'deploy':
            ...
            msg = 'PRECIP SUB 24 HR MATS METADATA START: ' + utcnow
            print(msg)
            regions_per_model_mats_all_categories('deploy')
            utcnow = str(datetime.now())
            msg = 'PRECIP SUB 24 HR MATS METADATA END: ' + utcnow
            print(msg)
