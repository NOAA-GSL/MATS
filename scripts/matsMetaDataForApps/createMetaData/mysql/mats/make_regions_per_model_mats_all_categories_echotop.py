#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in echotop
...
def regions_per_model_mats_all_categories(mode):
    ...
    for model in all_data_sources:
        ...
        if len(these_regions) > 0 and len(these_fcst_lens) > 0 and len(these_trshs) > 0:
            update_rpm_record(cnx, cursor, model, display_text, these_regions, these_fcst_lens, these_trshs, these_scales, cat, do, catstats['mindate'], catstats['maxdate'], catstats['numrecs'])
