#!/usr/bin/env python
#
# Creates a regions_per_model_mats_all_categories table for all models in ruc_ua_sums2
...
def regions_per_model_mats_all_categories(mode):
    ...
    skiptables = "all_display_categories all_display_categories_build all_display_categories_dev regions_per_model_mats_all_categories regions_per_model_mats_all_categories_dev regions_per_model_mats_all_categories_build template TABLESTATS_build Atemplate 1279152000 TABLESTATS clear cloudy"
    ...
    for row in cursor:
        ...
        if tablename not in skiptables and 'reg' in tablename and tablename[-1].isdigit():
            ...
            if '_Areg' in tablename:
                ...
                model = re.sub('_Breg.*', '', tablename)
            else:
                model = re.sub('_reg.*', '', tablename)
            ...
            per_table[tablename]['table_name_prefix'] = model_table_name_prefix
            region = re.sub(model_table_name_prefix, "", tablename)
            per_table[tablename]['region'] = region
            # print("model is " + model + ", region is " + region + ", model_table_name_prefix is " + model_table_name_prefix)
    ...
    if TScleaned:
        for tablename in per_table.keys():
            ...
            replace_tablestats_rec = "REPLACE INTO TABLESTATS_build (tablename, table_name_prefix, mindate, maxdate, minhour, maxhour, model, region, fcst_lens, numrecs) values( %s, %s,  %s, %s, %s, %s, %s, %s, %s, %s)"
            qd = []
            qd.append(str(tablename))
            qd.append(str(per_table[tablename]['table_name_prefix']))
            qd.append(str(stats['mindate']))
            qd.append(str(stats['maxdate']))
            qd.append(str(stats['minhour']))
            qd.append(str(stats['maxhour']))
    ...
    for model in all_data_sources:
        ...
        for row in cursor:
            this_table_name_prefix = row.values()[0]
