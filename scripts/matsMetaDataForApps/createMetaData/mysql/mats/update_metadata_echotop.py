#!/usr/bin/env python
#
# Updates the regions_per_model_mats_all_categories table for all models in echotop
...
def update_rpm_record(cnx, cursor, table_name, display_text, regions, fcstlens, trshs, scales, display_category,
...
def reprocess_specific_metadata(models_to_reprocess):
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_lens']) > 0 and len(per_model[model]['trshs']) > 0 and len(per_model[model]['scale']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['region'], per_model[model]['fcst_lens'], per_model[model]['trshs'], per_model[model]['scale'], per_model[model]['display_category'], per_model[model]['display_order'], per_model[model]['mindate'], per_model[model]['maxdate'], per_model[model]['numrecs'])
