#!/usr/bin/env python
"""
This script updates specified db/model combinations in the metadata tables required for a METexpress surface app.
...
Usage: ./selective_update_MEsurface.py path_to_file.cnf db1.model1,db2.model2,...
...
import metexpress.MEsurface as MEsurface

class UpdateMESurface:
    ...
    def update_tables_and_close_cnx(self):
        print("selective_MEsurface - Publishing metadata")
    ...
    def build_stats_object(self):
        ...
        for mvdb in mvdbs:
            ...
            self.cnx.commit()
            print("\n\nselective_MEsurface - Using db " + mvdb)
            ...
            for model in mvdb_map[mvdb]:
                ...
                if int(per_mvdb[mvdb][model]['numrecs']) > int(0):
                    ...
                    print("selective_MEsurface - Getting regions for model " + model)
                    ...
                    print("selective_MEsurface - Getting levels for model " + model)
                    ...
                    print("selective_MEsurface - Getting variables for model " + model)
    ...
    def update_model_in_metadata_table(self, mvdb, model, raw_metadata):
        ...
        self.cursor2.execute(
            "select * from surface_mats_metadata where db = '" + mvdb + "' and model = '" + model + "';")
        ...
        if len(raw_metadata['regions']) > int(0) and len(raw_metadata['levels']) and len(raw_metadata['fcsts']) and len(
            ...
            if self.cursor2.rowcount == 0:
                update_statement = "insert into surface_mats_metadata_dev (db, model, display_text, regions, levels, fcst_lens, variables, fcst_orig, mindate, maxdate, numrecs, updated) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                qd.append(mvdb)
                qd.append(model)
                qd.append(display_text)
            else:
                update_statement = "update surface_mats_metadata_dev set regions = %s, levels = %s, fcst_lens = %s, variables = %s, fcst_orig = %s, mindate = %s, maxdate = %s, numrecs = %s, updated = %s where db = '" + mvdb + "' and model = '" + model + "';"
    ...
    def update_groups(self, mvdb):
        ...
        if update_needed:
            ...
            insert_group = 'insert into surface_database_groups_dev (db_group, dbs) values("' + str(
    ...
    def update(self,options):
        try:
            ...
            print('SURFACE MATS FOR MET UPDATE METADATA START: ' + utc_now)
            ...
            print('SURFACE MATS FOR MET UPDATE METADATA END: ' + utc_now)
    ...
    def get_options(self, args):
        ...
        UpdateMESurface.validate_options(options)
        return options


if __name__ == '__main__':
    options = UpdateMESurface.get_options(sys.argv)
    updater = UpdateMESurface(options['cnf_file'],options['metadata_database'])
