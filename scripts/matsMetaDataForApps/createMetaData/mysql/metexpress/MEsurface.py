...
class MESurface:
    ...
    def build_stats_object(self):
        ...
        for mvdb in mvdbs:
            ...
            self.cursor.execute(
                "select count(distinct stat_header_id) as header_id_count from stat_header where fcst_lev in('MSL','SFC','Z0','Z2','Z10','H0','H2','H10','L0') and fcst_var not regexp '^OZ|^PM25';")
            self.cnx.commit()
            ...
            for line in self.cursor:
                ...
                temp_fcsts = set()
                temp_fcsts_orig = set()
                get_stat_header_ids = "select stat_header_id from stat_header where model='" + model + "' and fcst_lev in('MSL','SFC','Z0','Z2','Z10','H0','H2','H10','L0') and fcst_var not regexp '^OZ|^PM25';"
                ...
                if needs_default_metadata:
                    ...
                    if stat_header_id_list is not None:
                        ...
                        for stat_header_id in stat_header_id_list:
                            get_stats = 'select min(fcst_valid_beg) as mindate, max(fcst_valid_beg) as maxdate, count(fcst_valid_beg) as numrecs from ' + self.line_data_table + ' where stat_header_id  = "' + str(stat_header_id) + '";'
    ...
    def get_options(self, args):
        ...
        MESurface.validate_options(options)
        return options


if __name__ == '__main__':
    print('SURFACE MATS FOR MET METADATA START: ' + str(datetime.now()))
    ...
    me_dbcreator.metadata_table = "surface_mats_metadata"
    ...
    me_dbcreator.database_groups = "surface_database_groups"
    ...
    print('SURFACE MATS FOR MET METADATA END: ' + str(datetime.now()))
    sys.exit(0)
