...
class MEAirquality:
    def mysql_prep_tables(self):
        try:
            ...
            print(" MEairquality - Error: " + str(e))
            traceback.print_stack()
            sys.exit(1)

        # see if the metadata database already exists
        print(" MEairquality - Checking for " + self.metadata_database)
        ...
        print(" MEairquality - Checking for air quality metadata tables")
        ...
        self.cnx.commit()
        if self.cursor.rowcount == 0:
            print(" MEairquality - Metadata prod table does not exist--creating it")
            create_table_query = 'create table airquality_mats_metadata like airquality_mats_metadata_dev;'
            self.cursor.execute(create_table_query)
            self.cnx.commit()

        print(" MEairquality - Deleting from metadata dev table")
        self.cursor.execute("delete from airquality_mats_metadata_dev;")
        self.cnx.commit()

        # see if the metadata group tables already exist
        self.cursor.execute('show tables like "airquality_database_groups_dev";')
        if self.cursor.rowcount == 0:
            print(" MEairquality - Database group dev table does not exist--creating it")
            create_table_query = 'create table airquality_database_groups_dev (db_group varchar(255), dbs varchar(32767));'
            self.cursor.execute(create_table_query)
            self.cnx.commit()
        self.cursor.execute('show tables like "airquality_database_groups";')
        if self.cursor.rowcount == 0:
            print("MEairquality - Database group prod table does not exist--creating it")
            create_table_query = 'create table airquality_database_groups like airquality_database_groups_dev;'
            self.cursor.execute(create_table_query)
            self.cnx.commit()

        print(" MEairquality - Deleting from group dev table")
        self.cursor.execute("delete from airquality_database_groups_dev;")
        self.cnx.commit()

    def deploy_dev_table_and_close_cnx(self):
        print(" MEairquality - Publishing metadata")
        self.cursor.execute("use  " + self.metadata_database + ";")
        self.cnx.commit()
    ...
    def build_stats_object(self):
        print(" MEairquality - Compiling metadata")
        ...
        for mvdb in mvdbs:
            ...
            self.cnx.commit()
            print("\n\n MEairquality - Using db " + mvdb)

            # Get the models in this database
            get_models = 'select distinct model from stat_header where fcst_var regexp "^OZ|^PM25";'
            self.cursor.execute(get_models)
            self.cnx.commit()
            for line in self.cursor:
                model = list(line.values())[0]
                per_mvdb[mvdb][model] = {}
                print("\n MEairquality - Processing model " + model)
                ...
                print(" MEairquality - Getting regions for model " + model)
                ...
                print(" MEairquality - Getting levels for model " + model)
                ...
                # Get the airquality variables for this model in this database
                ...
                print(" MEairquality - Getting variables for model " + model)
                ...
                print("MEairquality - Getting fcst lens for model " + model)
                temp_fcsts = set()
                temp_fcsts_orig = set()
                ...
                if int(per_mvdb[mvdb][model]['numrecs']) > int(0):
                    ...
                    print("\n MEairquality - Storing metadata for model " + model)
                    self.add_model_to_metadata_table(cnx3, cursor3, mvdb, model, per_mvdb[mvdb][model])
                else:
                    print("\n MEairquality - No valid metadata for model " + model)
        ...
        try:
            ...
            print(" MEairquality - Error closing 2nd cursor: " + str(e))
            traceback.print_stack()
        try:
            cursor3.close()
            cnx3.close()
        except pymysql.Error as e:
            print(" MEairquality - Error closing 3rd cursor: " + str(e))
            traceback.print_stack()
    ...
    def add_model_to_metadata_table(self, cnx_tmp, cursor_tmp, mvdb, model, raw_metadata):
        ...
        if len(raw_metadata['regions']) > int(0) and len(raw_metadata['levels']) and len(raw_metadata['fcsts']) and len(
            ...
            insert_row = "insert into airquality_mats_metadata_dev (db, model, display_text, regions, levels, fcst_lens, variables, trshs, fcst_orig, mindate, maxdate, numrecs, updated) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            qd.append(mvdb)
            qd.append(model)
    ...
    def populate_db_group_tables(self, db_groups):
        ...
        for group in db_groups:
            qd = []
            insert_row = "insert into airquality_database_groups_dev (db_group, dbs) values(%s, %s)"
            qd.append(group)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print(" MEairquality - Error -- mysql cnf file needs to be passed in as argument")
        sys.exit(1)
    elif len(sys.argv) == 2:
        cnf_file = sys.argv[1]
        exists = os.path.isfile(cnf_file)
        if exists:
            print(" MEairquality - using cnf file: " + cnf_file)
        else:
            print(" MEairquality - cnf file " + cnf_file + " is not a file - exiting")
            sys.exit(1)
    elif len(sys.argv) == 3:
        cnf_file = sys.argv[1]
        exists = os.path.isfile(cnf_file)
        ...
        print(" MEairquality - using matadata database: " + metadataDatabaseName)

    utc_now = str(datetime.now())
    msg = 'AIR QUALITY MATS FOR MET METADATA START: ' + utc_now
    print(msg)
    me_dbcreator = MEAirquality()
    me_dbcreator.main(cnf_file, metadataDatabaseName)
    utc_now = str(datetime.now())
    msg = 'AIR QUALITY MATS FOR MET METADATA END: ' + utc_now
    print(msg)
    sys.exit(0)
