#!/scratch3/BMC/amb-verif/miniconda/miniconda3/envs/avid_verify_py3_6/bin/python
#
...
def reprocess_specific_metadata(models_to_reprocess):
        ...
        for row in cursor:
            ...
            if table_model == model:
                ...
                if int(stats['numrecs']) > 0:
                    ...
                    get_fcst_types = (
                        "SELECT DISTINCT accum_len FROM " + tablename + ";")
                    cursor2.execute(get_fcst_types)
                    thisfcst_types = []
                    for row2 in cursor2:
                        val = int(list(row2.values())[0])
                        if val in [1, 6, 12]:
                            thisfcst_types.append(val)
                    per_model[model]['fcst_type'] = list(
                        set(per_model[model]['fcst_type']) | set(thisfcst_types))
                    per_model[model]['fcst_type'].sort(key=int)
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_type']) > 0 and len(per_model[model]['trshs']) > 0 and len(per_model[model]['scale']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['region'], per_model[model]['fcst_type'], per_model[model]['trshs'], per_model[model]
                              ['scale'], per_model[model]['display_category'], per_model[model]['display_order'], per_model[model]['mindate'], per_model[model]['maxdate'], per_model[model]['numrecs'])
