...
def reprocess_specific_metadata(models_to_reprocess):
        ...
        for row in cursor:
            ...
            if region in valid_regions.keys():
                ...
                if int(stats['numrecs']) > 0:
                    ...
                    get_fcst_lens = (
                        "SELECT DISTINCT fcst_len,fcst_min FROM " + tablename + ";")
                    cursor2.execute(get_fcst_lens)
                    thisfcst_lens = []
                    for row2 in cursor2:
                        val = int(row2['fcst_len'] * 60) + \
                            int(row2['fcst_min'])
                    ...
                    get_truths = (
                        "SELECT DISTINCT truth FROM " + tablename + ";")
                    cursor2.execute(get_truths)
                    thistruths = []
                    for row2 in cursor2:
                        val = list(row2.values())[0]
                        thistruths.append(val)
                    per_model[model]['truths'] = list(
                        set(per_model[model]['truths']) | set(thistruths))
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_len']) > 0 and len(per_model[model]['trshs']) > 0 and len(per_model[model]['truths']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['region'], per_model[model]['fcst_len'], per_model[model]['trshs'], per_model[model]
                              ['truths'], per_model[model]['display_category'], per_model[model]['display_order'], per_model[model]['mindate'], per_model[model]['maxdate'], per_model[model]['numrecs'])
