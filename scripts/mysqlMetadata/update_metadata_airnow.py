...
#
# Updates the regions_per_model_mats_all_categories table for all models in airnow_sums
...
def reprocess_specific_metadata(models_to_reprocess):
        ...
        for row in cursor:
            ...
            if region in valid_regions.keys():
                ...
                if int(stats['numrecs']) > 0:
                    ...
                    get_trsh = (
                        "SELECT DISTINCT thresh_10 FROM " + tablename + ";")
                    ...
                    get_scale = ("SELECT DISTINCT scale FROM " +
                                 tablename + ";")
                    cursor2.execute(get_scale)
                    thisscale = []
                    for row2 in cursor2:
                        val = list(row2.values())[0]
                        thisscale.append(int(val))
                    per_model[model]['scales'] = list(
                        set(per_model[model]['scales']) | set(thisscale))
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_len']) > 0 and len(per_model[model]['trshs']) > 0 and len(per_model[model]['scales']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['region'], per_model[model]['fcst_len'], per_model[model]['trshs'], per_model[model]['scales'],
