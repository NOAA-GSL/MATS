...
def reprocess_specific_metadata(models_to_reprocess):
        ...
        for row in cursor:
            ...
            if region in valid_regions.keys():
                ...
                if int(stats['numrecs']) > 0:
                    ...
                    get_trsh = ("SELECT DISTINCT thresh FROM " +
                                tablename + ";")
    ...
    for model in models_to_reprocess:
        if len(per_model[model]['region']) > 0 and len(per_model[model]['fcst_len']) > 0 and len(per_model[model]['trshs']) > 0 and len(per_model[model]['sources']) > 0:
            update_rpm_record(cnx, cursor, model, per_model[model]['display_text'], per_model[model]['region'], per_model[model]['sources'], per_model[model]['fcst_len'], per_model[model]
                              ['trshs'], per_model[model]['display_category'], per_model[model]['display_order'], per_model[model]['mindate'], per_model[model]['maxdate'], per_model[model]['numrecs'])
