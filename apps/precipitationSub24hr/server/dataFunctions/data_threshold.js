...
dataThreshold = function (plotParams, plotFunction) {
    ...
    for (var curveIndex = 0; curveIndex < curvesLength; curveIndex++) {
        ...
        if (diffFrom == null) {
            ...
            var finishMoment;
            try {
                queryResult = matsDataUtils.queryThresholdDB(sumPool, statement);
                finishMoment = moment();
            ...
            const diffResult = matsDataUtils.getDataForThresholdDiffCurve({
                dataset: dataset,
                ymin: ymin,
                ymax: ymax,
                diffFrom: diffFrom
        ...
        const cOptions = matsDataUtils.generateThresholdCurveOptions(curve, curveIndex, axisMap, d);  // generate plot with data, curve annotation, axis labels, etc.
    ...
    if (curvesLength > 1 && (plotParams['plotAction'] === matsTypes.PlotActions.matched)) {
        ...
        for (curveIndex = 0; curveIndex < curvesLength; curveIndex++) { // every curve
            threshGroups[curveIndex] = [];
            subSecs[curveIndex] = {};
            var data = dataset[curveIndex].data;
            for (var di = 0; di < data.length; di++) { // every thresh
                currThresh = data[di][0];
                subSecs[curveIndex][currThresh] = data[di][4]; //store raw secs for each threshold
                threshGroups[curveIndex].push(currThresh);
            }
        }
        var matchingThreshs = _.intersection.apply(_, threshGroups);
        var subSecIntersection = {};

        for (var fi = 0; fi < matchingThreshs.length; fi++) { // every thresh
            currThresh = matchingThreshs[fi];
            var currSubSecIntersection = subSecs[0][currThresh];   //fill current intersection array with secs from the first curve
            for (curveIndex = 1; curveIndex < curvesLength; curveIndex++) { // every curve
                currSubSecIntersection = _.intersection(currSubSecIntersection,subSecs[curveIndex][currThresh]);   //take intersection of current secs and previously matched secs
            }
            subSecIntersection[currThresh] = currSubSecIntersection;   //store final current intersection array for each thresh
        }
    ...
    for (curveIndex = 0; curveIndex < curvesLength; curveIndex++) { // every curve
        ...
        while (di < data.length) {
            if ((plotParams['plotAction'] === matsTypes.PlotActions.matched && curvesLength > 1) && matchingThreshs.indexOf(data[di][0]) === -1) {
            ...
            if (plotParams['plotAction'] === matsTypes.PlotActions.matched && curvesLength > 1 && sub_secs.length > 0) {
                ...
                for (var si = 0; si < sub_secs.length; si++) {  //loop over all sub values for this thresh
                    if (subSecIntersection[currThresh].indexOf(sub_secs[si]) !== -1) { //store the sub-value only if its associated sec is in the matching array for this thresh
            ...
            threshs.push(data[di][0]);  // inverted data for graphing - remember?
    ...
    const resultOptions = matsDataUtils.generateThresholdPlotOptions(dataset, curves, axisMap, errorMax);
