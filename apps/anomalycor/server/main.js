...
const doPlotParams = function () {
  ...
  if (matsCollections.PlotParams.find().count() == 0) {
    ...
    const xOptionsMap = {
      ...
      "Init UTC hour":
        "select (unix_timestamp(m0.valid_date)+3600*m0.valid_hour-m0.fcst_len*3600)%(24*3600)/3600 as xVal, ",
      "Valid Date": "select unix_timestamp(m0.valid_date)+3600*m0.valid_hour as xVal, ",
      "Init Date":
        "select unix_timestamp(m0.valid_date)+3600*m0.valid_hour-m0.fcst_len*3600 as xVal, ",
    };
    ...
    const yOptionsMap = {
      ...
      "Init UTC hour":
        "(unix_timestamp(m0.valid_date)+3600*m0.valid_hour-m0.fcst_len*3600)%(24*3600)/3600 as yVal, ",
      "Valid Date": "unix_timestamp(m0.valid_date)+3600*m0.valid_hour as yVal, ",
      "Init Date":
        "unix_timestamp(m0.valid_date)+3600*m0.valid_hour-m0.fcst_len*3600 as yVal, ",
    };
...
const doCurveParams = function () {
  ...
  if (matsCollections["valid-time"].findOne({ name: "valid-time" }) == undefined) {
    const optionsMap = {
      both: [""],
      "0-UTC": ["and m0.valid_hour = 0"],
      "12-UTC": ["and m0.valid_hour = 12"],
    };
  ...
  if (matsCollections.average.findOne({ name: "average" }) == undefined) {
    const optionsMap = {
      None: [
        `ceil(${3600}*floor(((unix_timestamp(m0.valid_date)+3600*m0.valid_hour)+${3600}/2)/${3600}))`,
      ],
      "1D": [
        `ceil(${3600 * 24}*floor(((unix_timestamp(m0.valid_date)+3600*m0.valid_hour)+${
          3600 * 24
        }/2)/${3600 * 24}))`,
      ],
      "3D": [
        `ceil(${
          3600 * 24 * 3
        }*floor(((unix_timestamp(m0.valid_date)+3600*m0.valid_hour)+${
          3600 * 24 * 3
        }/2)/${3600 * 24 * 3}))`,
      ],
      "7D": [
