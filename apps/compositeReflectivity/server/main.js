...
const doCurveParams = function () {
    ...
    if (matsCollections.CurveParams.find({name: 'statistic'}).count() == 0) {
        var optionsMap = {
            ...
            'PODy (POD of reflectivity < threshold)': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.yn)) * 100 as stat', 'x100'],

            'PODn (POD of reflectivity > threshold)': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.ny)) * 100 as stat', 'x100'],
