  ...
  for (var curveIndex = 0; curveIndex < curvesLength; curveIndex++) {
    ...
    if (forecastLength === matsTypes.ForecastTypes.dieoff) {
      ...
      if (utcCycleStart.length !== 0 && utcCycleStart !== matsTypes.InputTypes.unused) {
        utcCycleStartClause =
          'and floor(((unix_timestamp(m0.date)+3600*m0.hour) - m0.fcst_len*3600)%(24*3600)/3600) IN(' +
          utcCycleStart +
          ')';
      }
      dateClause =
        'and unix_timestamp(m0.date)+3600*m0.hour-m0.fcst_len*3600 >= ' +
        fromSecs +
        ' and unix_timestamp(m0.date)+3600*m0.hour-m0.fcst_len*3600 <= ' +
        toSecs;
    } else {
      dateClause =
        'and unix_timestamp(m0.date)+3600*m0.hour-m0.fcst_len*3600 = ' + fromSecs;
    }
    ...
    var statisticClause =
      ...
      variable[5] +
      ') as abs_sum, ' +
      "group_concat(unix_timestamp(m0.date)+3600*m0.hour, ';', " +
      levelVar +
      ", ';', " +
      variable[0] +
      ", ';', " +
