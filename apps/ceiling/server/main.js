...
const doCurveParams = function () {
    ...
    try {
        rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,regions,display_text,fcst_lens,trsh,mindate,maxdate from regions_per_model_mats_all_categories order by display_category, display_order;");
    ...
    if (matsCollections.CurveParams.find({name: 'statistic'}).count() == 0) {
        var optionsMap = {
            ...
            'PODy (POD of ceiling < threshold)': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.ny)) * 100 as stat, group_concat(((m0.yy)/(m0.yy+m0.ny)) * 100, ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'x100', 100],

            'PODn (POD of ceiling > threshold)': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.yn)) * 100 as stat, group_concat(((m0.nn)/(m0.nn+m0.yn)) * 100, ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'x100', 100],
            ...
            'Nlow (obs < threshold, avg per hr)': ['avg(m0.yy+m0.ny+0.000) as stat, group_concat((m0.yy+m0.ny), ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'Number', null],

            'Nhigh (obs > threshold, avg per hr)': ['avg(m0.nn+m0.yn+0.000) as stat, group_concat((m0.nn+m0.yn), ";", m0.time order by m0.time) as sub_data, count(m0.nn) as N0', 'Number', null],

            'Ntot (total obs, avg per hr)': ['avg(m0.yy+m0.yn+m0.ny+m0.nn+0.000) as stat, group_concat((m0.yy+m0.yn+m0.ny+m0.nn), ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'Number', null],
    ...
    if (matsCollections.CurveParams.find({name: 'average'}).count() == 0) {
        optionsMap = {
            'None': ['m0.time'],
            '3hr': ['ceil(' + 60 * 60 * 3 + '*floor((m0.time)/' + 60 * 60 * 3 + ')+' + 60 * 60 * 3 + '/2)'],
            '6hr': ['ceil(' + 60 * 60 * 6 + '*floor((m0.time)/' + 60 * 60 * 6 + ')+' + 60 * 60 * 6 + '/2)'],
            '12hr': ['ceil(' + 60 * 60 * 12 + '*floor((m0.time)/' + 60 * 60 * 12 + ')+' + 60 * 60 * 12 + '/2)'],
...
Meteor.startup(function () {
    ...
    matsMethods.resetApp({appMdr:mdr, appType:matsTypes.AppTypes.mats, app:'ceiling'});
});
