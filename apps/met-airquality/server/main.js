...
const doCurveParams = function () {
    ...
    try {
        for (var k = 0; k < myDBs.length; k++) {
            ...
            rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,display_text,line_data_table,variable,regions,levels,fcst_lens,fcst_orig,trshs,mindate,maxdate from airquality_mats_metadata where db = '" + thisDB + "' group by model,display_text,line_data_table,variable,regions,levels,fcst_lens,fcst_orig,trshs,mindate,maxdate order by model,line_data_table,variable;");
            for (i = 0; i < rows.length; i++) {
        ...
        matsCollections.CurveParams.insert(
            {
                ...
                superiorNames: ['database', 'data-source', 'plot-type'],
                dependentNames: ["region", "forecast-length", "level", "threshold"],
                controlButtonCovered: true,
                unique: false,
