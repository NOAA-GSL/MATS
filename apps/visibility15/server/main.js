...
const doCurveParams = function () {
    ...
    if (matsCollections.CurveParams.find({name: 'statistic'}).count() == 0) {
        const optionsMap = {
            ...
            'PODy (POD of visibility < threshold)': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.ny)) * 100 as stat, group_concat(((m0.yy)/(m0.yy+m0.ny)) * 100, ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'x100', 100],

            'PODn (POD of visibility > threshold)': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.yn)) * 100 as stat, group_concat(((m0.nn)/(m0.nn+m0.yn)) * 100, ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'x100', 100],
            ...
            'Nlow (obs < threshold, avg per 15 min)': ['avg(m0.yy+m0.ny+0.000) as stat, group_concat((m0.yy+m0.ny), ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'Number', null],

            'Nhigh (obs > threshold, avg per 15 min)': ['avg(m0.nn+m0.yn+0.000) as stat, group_concat((m0.nn+m0.yn), ";", m0.time order by m0.time) as sub_data, count(m0.nn) as N0', 'ctc', 'Number', null],

            'Ntot (total obs, avg per 15 min)': ['avg(m0.yy+m0.yn+m0.ny+m0.nn+0.000) as stat, group_concat((m0.yy+m0.yn+m0.ny+m0.nn), ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'Number', null],
            ...
            'N in average (to nearest 100)': ['sum(m0.yy+m0.ny+m0.yn+m0.nn+0.000) as stat, group_concat((m0.yy+m0.ny+m0.yn+m0.nn), ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'Number', null]
        };
        matsCollections.CurveParams.insert(
            {
                name: 'statistic',
    ...
    if (matsCollections.CurveParams.find({name: 'x-axis-parameter'}).count() == 0) {
        ...
        const optionsMap = {
            ...
            'Threshold': "select m0.trsh/100 as xVal, ",    // produces thresholds in mi
    ...
    if (matsCollections.CurveParams.find({name: 'y-axis-parameter'}).count() == 0) {
        ...
        const optionsMap = {
            ...
            'Threshold': "m0.trsh/100 as yVal, ",    // produces thresholds in mi
