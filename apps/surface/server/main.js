...
const doCurveParams = function () {
    ...
    try {
        const rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,metar_string,regions,display_text,fcst_lens,mindate,maxdate from regions_per_model_mats_all_categories order by display_category, display_order;");
    ...
    if (matsCollections["average"].findOne({name: 'average'}) == undefined) {
        const optionsMap = {
            ...
            '3D': ['ceil(' + 3600 * 24 * 3 + '*floor((({{timeVar}})+' + 3600 * 24 * 3 + '/2)/' + 3600 * 24 * 3 + '))'],
            '7D': ['ceil(' + 3600 * 24 * 7 + '*floor((({{timeVar}})+' + 3600 * 24 * 7 + '/2)/' + 3600 * 24 * 7 + '))'],
            '30D': ['ceil(' + 3600 * 24 * 30 + '*floor((({{timeVar}})+' + 3600 * 24 * 30 + '/2)/' + 3600 * 24 * 30 + '))'],
            '60D': ['ceil(' + 3600 * 24 * 60 + '*floor((({{timeVar}})+' + 3600 * 24 * 60 + '/2)/' + 3600 * 24 * 60 + '))'],
            '90D': ['ceil(' + 3600 * 24 * 90 + '*floor((({{timeVar}})+' + 3600 * 24 * 90 + '/2)/' + 3600 * 24 * 90 + '))'],
            '180D': ['ceil(' + 3600 * 24 * 180 + '*floor((({{timeVar}})+' + 3600 * 24 * 180 + '/2)/' + 3600 * 24 * 180 + '))'],
        };
...
const doCurveTextPatterns = function () {
    ...
    if (matsCollections.CurveTextPatterns.find().count() == 0) {
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "data-source", "region-type", "region", "statistic", "variable", "average", "forecast-length", "valid-time", "truth", "sites", "sitesMap"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "data-source", "region-type", "region", "statistic", "variable", "dieoff-type", "valid-time", "utc-cycle-start", "truth", "sites", "sitesMap", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "data-source", "region-type", "region", "statistic", "variable", "forecast-length", "truth", "sites", "sitesMap", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "data-source", "region-type", "region", "statistic", "variable", "utc-cycle-start", "truth", "sites", "sitesMap"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
