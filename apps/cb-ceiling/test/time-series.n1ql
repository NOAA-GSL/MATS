SELECT m0.fcstValidEpoch AS AVTIME,
    ...
    SUM(
        CASE
            WHEN m0data.Ceiling < 500
            AND odata.Ceiling < 500 THEN 1
            ELSE 0
        END
    ) AS hit,
    SUM(
        CASE
            WHEN m0data.Ceiling < 500
            AND NOT odata.Ceiling < 500 THEN 1
            ELSE 0
        END
    ) AS fa,
    SUM(
        CASE
            WHEN NOT m0data.Ceiling < 500
            AND odata.Ceiling < 500 THEN 1
            ELSE 0
        END
    ) AS miss, 
    ...
    SUM(
        CASE
            WHEN m0data.Ceiling IS NOT MISSING
            AND odata.Ceiling IS NOT MISSING THEN 1
            ELSE 0
        END
    ) AS N0,
    ARRAY_AGG(
        TO_STRING(m0.fcstValidEpoch) || ';' || CASE
            WHEN m0data.Ceiling < 500
            AND odata.Ceiling < 500 THEN '1'
            ELSE '0'
        END || ';' || CASE
...
FROM mdata AS m0 USE INDEX (
        ix_subset_version_model_fcstLen_fcstValidEpoc
    )
    JOIN mdata AS o USE INDEX(
        adv_fcstValidEpoch_docType_subset_version_type
...
WHERE o.type = 'DD'
    ...
    AND m0data.name IN ['KGKY',
    'KY19',
    'KY49',
    'KY51']
    AND odata.name IN ['KGKY',
    'KY19',
    'KY49',
    'KY51']
