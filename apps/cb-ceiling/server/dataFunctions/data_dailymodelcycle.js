...
dataDailyModelCycle = function (plotParams, plotFunction) {
  ...
  for (var curveIndex = 0; curveIndex < curvesLength; curveIndex++) {
    ...
    var utcCycleStartClause =
      'and (m0.fcstValidEpoch - m0.fcstLen*3600)%(24*3600)/3600 IN[' +
      utcCycleStart +
      ']';
    ...
    if (regionType === 'Predefined region') {
      ...
      statisticClause =
        "sum(m0.data.['" +
        threshold +
        "'].hits) hit, sum(m0.data.['" +
        threshold +
        "'].false_alarms) fa, " +
        "sum(m0.data.['" +
        threshold +
        "'].misses) miss, sum(m0.data.['" +
        threshold +
        "'].correct_negatives) cn, " +
        "ARRAY_SORT(ARRAY_AGG(TO_STRING(m0.fcstValidEpoch) || ';' || TO_STRING(m0.data.['" +
        threshold +
        "'].hits) || ';' || " +
        "TO_STRING(m0.data.['" +
        threshold +
        "'].false_alarms) || ';' || TO_STRING(m0.data.['" +
        threshold +
        "'].misses) || ';' || " +
        "TO_STRING(m0.data.['" +
        threshold +
        "'].correct_negatives))) sub_data, count(m0.data.['" +
        threshold +
        "'].hits) N0 ";
      dateClause =
        'and m0.fcstValidEpoch >= ' + fromSecs + ' and m0.fcstValidEpoch <= ' + toSecs;
      whereClause =
        'WHERE ' +
        "m0.type='DD' " +
        "AND m0.docType='CTC' " +
      ...
      if (sitesList.length > 0 && sitesList !== matsTypes.InputTypes.unused) {
        ...
        sitesClause =
          sitesClause + " and odata.name in ['" + sitesList.join("','") + "']";
      ...
      statisticClause =
        ...
        threshold +
        ' ' +
        'AND NOT odata.Ceiling < ' +
        threshold +
        ' THEN 1 ELSE 0 END) AS fa, ' +
        'SUM(CASE WHEN NOT m0data.Ceiling < ' +
        threshold +
        ' ' +
        'AND odata.Ceiling < ' +
        threshold +
        ...
        "ARRAY_AGG(TO_STRING(m0.fcstValidEpoch) || ';' || CASE WHEN m0data.Ceiling < " +
        threshold +
        ' ' +
        'AND odata.Ceiling < ' +
        threshold +
        " THEN '1' ELSE '0' END || ';' || CASE WHEN m0data.Ceiling < " +
        threshold +
        ' ' +
        'AND NOT odata.Ceiling < ' +
        threshold +
        " THEN '1' ELSE '0' END || ';' || CASE WHEN NOT m0data.Ceiling < " +
        threshold +
        ' ' +
        'AND odata.Ceiling < ' +
        threshold +
        ...
        'AND NOT odata.Ceiling < ' +
        threshold +
        " THEN '1' ELSE '0' END) AS sub_data ";
      dateClause =
        'and m0.fcstValidEpoch >= ' +
        fromSecs +
        ' and m0.fcstValidEpoch <= ' +
        toSecs +
      ...
      whereClause =
        ...
        "AND m0.version='V01' ";
      siteDateClause =
        'and o.fcstValidEpoch >= ' + fromSecs + ' and o.fcstValidEpoch <= ' + toSecs;
      siteWhereClause =
        "WHERE o.type='DD' " +
        "AND o.docType='obs' " +
        "AND o.subset='METAR' " +
