...
const doCurveParams = function () {
    ...
    if (matsCollections.CurveParams.find({name: 'statistic'}).count() == 0) {
        var optionsMap = {
            'TSS (True Skill Score)': ['((sum(m0.yy)*sum(m0.nn) - sum(m0.yn)*sum(m0.ny))/((sum(m0.yy)+sum(m0.ny))*(sum(m0.yn)+sum(m0.nn)))) * 100 as stat, group_concat(((m0.yy*m0.nn - m0.yn*m0.ny)/((m0.yy+m0.ny)*(m0.yn+m0.nn))) * 100 order by m0.valid_time) as sub_values0, group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'x100', 100],

            'PODy (POD of visibility < threshold)': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.ny)) * 100 as stat, group_concat(((m0.yy)/(m0.yy+m0.ny)) * 100 order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'x100', 100],

            'PODn (POD of visibility > threshold)': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.yn)) * 100 as stat, group_concat(((m0.nn)/(m0.nn+m0.yn)) * 100 order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'x100', 100],

            'FAR (False Alarm Ratio)': ['((sum(m0.yn)+0.00)/sum(m0.yn+m0.yy)) * 100 as stat, group_concat(((m0.yn)/(m0.yn+m0.yy)) * 100 order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'x100', 0],

            'Bias (forecast/actual)': ['((sum(m0.yy+m0.yn)+0.00)/sum(m0.yy+m0.ny)) as stat, group_concat(((m0.yy+m0.yn)/(m0.yy+m0.ny)) order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'Ratio', 1],

            'CSI (Critical Success Index)': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.ny+m0.yn)) * 100 as stat, group_concat(((m0.yy)/(m0.yy+m0.ny+m0.yn)) * 100 order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'x100', 100],

            'HSS (Heidke Skill Score)': ['(2*(sum(m0.nn+0.00)*sum(m0.yy)-sum(m0.ny)*sum(m0.yn))/((sum(m0.nn+0.00)+sum(m0.yn))*(sum(m0.yn)+sum(m0.yy))+(sum(m0.nn+0.00)+sum(m0.ny))*(sum(m0.ny)+sum(m0.yy)))) * 100 as  stat, group_concat((2*(m0.nn*m0.yy - m0.ny*m0.yn) / ((m0.nn+m0.yn)*(m0.yn+m0.yy) + (m0.nn+m0.ny)*(m0.ny+m0.yy))) * 100 order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'x100', 100],

            'ETS (Equitable Threat Score)': ['(sum(m0.yy)-(sum(m0.yy+m0.yn)*sum(m0.yy+m0.ny)/sum(m0.yy+m0.yn+m0.ny+m0.nn)))/(sum(m0.yy+m0.yn+m0.ny)-(sum(m0.yy+m0.yn)*sum(m0.yy+m0.ny)/sum(m0.yy+m0.yn+m0.ny+m0.nn))) * 100 as stat, group_concat((m0.yy-((m0.yy+m0.yn)*(m0.yy+m0.ny)/(m0.yy+m0.yn+m0.ny+m0.nn)))/((m0.yy+m0.yn+m0.ny)-((m0.yy+m0.yn)*(m0.yy+m0.ny)/(m0.yy+m0.yn+m0.ny+m0.nn))) * 100 order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'x100', 100],

            'Nlow (obs < threshold, avg per hr)': ['avg(m0.yy+m0.ny+0.000) as stat, group_concat((m0.yy+m0.ny) order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'Number', null],

            'Nhigh (obs > threshold, avg per hr)': ['avg(m0.nn+m0.yn+0.000) as stat, group_concat((m0.nn+m0.yn) order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.nn) as N0', 'Number', null],

            'Ntot (total obs, avg per hr)': ['avg(m0.yy+m0.yn+m0.ny+m0.nn+0.000) as stat, group_concat((m0.yy+m0.yn+m0.ny+m0.nn) order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'Number', null],

            'Ratio (Nlow / Ntot)': ['(sum(m0.yy+m0.ny+0.000)/sum(m0.yy+m0.yn+m0.ny+m0.nn+0.000)) as stat, group_concat(((m0.yy+m0.ny)/(m0.yy+m0.yn+m0.ny+m0.nn)) order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'Ratio', null],

            'Ratio (Nhigh / Ntot)': ['(sum(m0.nn+m0.yn+0.000)/sum(m0.yy+m0.yn+m0.ny+m0.nn+0.000)) as stat, group_concat(((m0.nn+m0.yn)/(m0.yy+m0.yn+m0.ny+m0.nn)) order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.nn) as N0', 'Ratio', null],

            'N in average (to nearest 100)': ['sum(m0.yy+m0.ny+m0.yn+m0.nn+0.000) as stat, group_concat((m0.yy+m0.ny+m0.yn+m0.nn) order by m0.valid_time) as sub_values0 ,group_concat(m0.valid_time order by m0.valid_time) as sub_secs0, count(m0.yy) as N0', 'Number', null]
        };
        matsCollections.CurveParams.insert(
            {
                name: 'statistic',
