...
"title"        | user name  scorecard name  submit date  processed date daterange |
...
results: {
    "title":user + " " + name + " " + submitDate + " " + processedDate + " " + daterange,
    rows: {
        Rowlabel1: {
...
*/
    ...
    const submitEpoch = Math.floor(new Date() / 1000);
    let processedAt = 0; // should be filled in when processing is finished
    let user = plotParams['user'];
    let name = plotParams["scorecard-name"];
    let singleCurveParamNames = matsParamUtils.getSingleSelectCurveParamNames();
    let multiCurvaeParamNames = matsParamUtils.getMultiSelectCurveParamNames();
    // We are thinking that the combination of user/scorecardName/submitEpoch/processedEpoch is uniq

    let interval = {};
    if (plotParams["scorecard-schedule-mode"] == 'Once' === "Recurring") {
        switch (plotParams["scorecard-recurrence-interval"]) {
            case "Daily":
                interval = {"type":"daily", "hours":plotParams["these-hours-of-the-day"]};
                break;
            case "Weekly":
                interval = {"type":"weekly", "hours":plotParams["these-hours-of-the-day"],"daysOfWeek":plotParams["these-days-of-the-week"]};
                break;
            case "Monthly":
                interval = {"type":"monthly", "hours":plotParams["these-hours-of-the-day"], "daysOfMonth":plotParams["these-days-of-the-month"]};
                break;
            case "Yearly":
                interval = {"type":"yearly", "hours":plotParams["these-hours-of-the-day"], "daysOfMonth":plotParams["these-days-of-the-month"],"months":plotParams["these-months"]};
                break;
        }
    }
    ...
    let id = "SC:" +  user + ":" + name + ":" + submitEpoch + ":" + processedAt + ":" + idDateRange;
    let title =  user + ":" + name + ":" + submitEpoch + ":" + processedAt + ":" + dateRange;
    ...
    let scorecardDocument = {
        ...
        'significanceThresholds': plotParams['scorecard-percent-stdv'] === "Percent" ?
        {
    ...
    plotParams['curves'].forEach(function(curve){
        ...
        scorecardDocument['results']['rows'][curve['label']]['rowTitle'] = {'label':label,'datasource':curve['data-source'], 'validationDatasource':curve['validation-data-source']};
        ...
        curve['statistic'].forEach(function(stat){
            curve['variable'].forEach(function(variable){
                ...
                scorecardDocument['results']['rows'][curve['label']]['data'][stat + '-' + variable] = {};
                fcstLengths.forEach(function(fcstlen, index){
                    if (scorecardDocument['results']['rows'][curve['label']]['fcstlens'].includes(fcstlen)) {
                        scorecardDocument['results']['rows'][curve['label']]['data'][stat + '-' + variable][fcstlen] = [null, 'u'];
                    } else {
                        //mark this undefined
                        scorecardDocument['results']['rows'][curve['label']]['data'][stat + '-' + variable][fcstlen] = "undefined";
                    }
                });
            });
        });
