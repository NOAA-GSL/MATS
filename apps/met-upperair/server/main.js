...
const doCurveParams = function () {
    ...
    var dbArr;
    try {
        rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select * from upperair_database_groups order by db_group;");
    ...
    var thisDB;
    try {
        rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select distinct db from upperair_mats_metadata;");
    ...
    try {
        for (var k = 0; k < myDBs.length; k++) {
            ...
            rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,display_text,regions,levels,fcst_lens,fcst_orig,variables,mindate,maxdate from upperair_mats_metadata where db = '" + thisDB + "' group by model,display_text,regions,levels,fcst_lens,fcst_orig,variables,mindate,maxdate order by model;");
            for (i = 0; i < rows.length; i++) {
...
const doCurveTextPatterns = function () {
    ...
    if (matsCollections.CurveTextPatterns.find().count() == 0) {
        ...
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "group", "database", "data-source", "region", "statistic", "variable", "valid-time", "forecast-length", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
...
Meteor.startup(function () {
    ...
    try {
        matsMethods.resetApp({appMdr: mdr, appType: matsTypes.AppTypes.metexpress, app: 'met-upperair', title: "MET Upper Air", group: "METexpress"});
    } catch(error) {
