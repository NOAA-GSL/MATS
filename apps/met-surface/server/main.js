...
const doCurveParams = function () {
    ...
    var dbArr;
    try {
        rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select * from surface_database_groups order by db_group;");
    ...
    try {
        for (var k = 0; k < myDBs.length; k++) {
            ...
            rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,display_text,regions,levels,fcst_lens,fcst_orig,variables,mindate,maxdate from surface_mats_metadata where db = '" + thisDB + "' group by model,display_text,regions,levels,fcst_lens,fcst_orig,variables,mindate,maxdate order by model;");
            for (i = 0; i < rows.length; i++) {
...
Meteor.startup(function () {
    ...
    const mdr = new matsTypes.MetaDataDBRecord(matsTypes.DatabaseRoles.SUMS_DATA, "sumPool", "mats_metadata", ['surface_mats_metadata', 'surface_database_groups']);
    try {
        matsMethods.resetApp({appMdr: mdr, appType: matsTypes.AppTypes.metexpress, app: 'met-surface', title: "MET Surface", group: "METexpress"});
    } catch(error) {
