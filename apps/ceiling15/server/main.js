    ...
    if (matsCollections.CurveParams.find({name: 'data-source'}).count() == 0) {
        ...
        if (matsCollections.CurveParams.find({name: 'statistic'}).count() == 0) {
            var optionsMap = {
                ...
                'Bias (Forecast low cigs/actual)': ['((sum(m0.yy+m0.ny)+0.00)/sum(m0.yy+m0.yn)) as stat, group_concat(((m0.yy+m0.ny)/(m0.yy+m0.yn)) order by m0.time) as sub_values0 ,group_concat(m0.time order by m0.time) as sub_secs0, count(m0.yy) as N0', 'Ratio', 100],
