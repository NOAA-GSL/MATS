...
dataDieOff = function (plotParams, plotFunction) {
  ...
  for (var curveIndex = 0; curveIndex < curvesLength; curveIndex++) {
    ...
    if (regionType === 'Predefined region') {
      ...
      queryTableClause =
        'from ' + databaseRef.sumsDB + '.' + model + '_' + region + ' as m0';
      if (variable === '15 Minute Visibility') {
    ...
    if (forecastLength === matsTypes.ForecastTypes.dieoff) {
      ...
      if (validTimes.length !== 0 && validTimes !== matsTypes.InputTypes.unused) {
        validTimeClause =
          'and floor((m0.time+450)%(24*3600)/900)/4 IN(' + validTimes + ')';
      }
      ...
      if (utcCycleStart.length !== 0 && utcCycleStart !== matsTypes.InputTypes.unused) {
        utcCycleStartClause =
          'and floor(((m0.time+450) - (m0.fcst_len*60+m0.fcst_min)*60)%(24*3600)/900)/4 IN(' +
          utcCycleStart +
          ')';
      }
      if (regionType === 'Predefined region') {
        dateClause =
          'and m0.time-(m0.fcst_len*60+m0.fcst_min)*60 >= ' +
          fromSecs +
          ' and m0.time-(m0.fcst_len*60+m0.fcst_min)*60 <= ' +
          toSecs;
      } else {
        dateClause =
          'and m0.time-(m0.fcst_len*60+m0.fcst_min)*60 >= ' +
          fromSecs +
          ' - 300 and m0.time-(m0.fcst_len*60+m0.fcst_min)*60 <= ' +
          toSecs +
          ' + 300';
      }
    } else {
