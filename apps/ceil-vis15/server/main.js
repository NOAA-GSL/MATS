...
const doCurveParams = function () {
    ...
    try {
        for (didx = 0; didx < variables.length; didx++) {
            ...
            rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select trsh,description from " + variableDBNames[variables[didx]].modelDB + ".threshold_descriptions;");
    ...
    try {
        for (didx = 0; didx < variables.length; didx++) {
            ...
            if (variables[didx] === "15 Minute Visibility") {
                rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select truth,description from " + variableDBNames[variables[didx]].modelDB + ".truth_descriptions;");
                var masterDescription;
                var masterTruth;
                for (var j = 0; j < rows.length; j++) {
                    masterDescription = rows[j].description.trim();
    ...
    try {
        for (didx = 0; didx < variables.length; didx++) {
            ...
            if (variables[didx] === "15 Minute Visibility") {
                rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,regions,display_text,fcst_lens,trsh,truth,mindate,maxdate from " + variableDBNames[variables[didx]].sumsDB + ".regions_per_model_mats_all_categories order by display_category, display_order;");
            } else {
                rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,regions,display_text,fcst_lens,trsh,mindate,maxdate from " + variableDBNames[variables[didx]].sumsDB + ".regions_per_model_mats_all_categories order by display_category, display_order;");
            }
            for (var i = 0; i < rows.length; i++) {
                ...
                modelDateRangeMap[variables[didx]][model] = {minDate: rowMinDate, maxDate: rowMaxDate};
    ...
    try {
        ...
        const rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select madis_id,name,lat,lon,elev,description from madis3.metars_mats_global where lat > -16380 and lat < 16380 and lon > -32760 and lon < 32760 order by name;");
        for (var i = 0; i < rows.length; i++) {
        ...
        matsCollections["region"].insert(
            {
                ...
                options: regionModelOptionsMap[variables[0]][Object.keys(regionModelOptionsMap[variables[0]])[0]],
                valuesMap: masterRegionValuesMap,
                superiorNames: ['variable', 'data-source'],
                controlButtonCovered: true,
                unique: false,
                default: regionModelOptionsMap[variables[0]][Object.keys(regionModelOptionsMap[variables[0]])[0]][0],
                controlButtonVisibility: 'block',
                displayOrder: 2,
                displayPriority: 1,
                displayGroup: 2
        ...
        matsCollections["threshold"].insert(
            {
                ...
                options: thresholdsModelOptionsMap[variables[0]][Object.keys(thresholdsModelOptionsMap[variables[0]])[0]],
                valuesMap: masterThresholdValuesMap,
                superiorNames: ['variable', 'data-source'],
                controlButtonCovered: true,
                unique: false,
                default: thresholdsModelOptionsMap[variables[0]][Object.keys(thresholdsModelOptionsMap[variables[0]])[0]][0],
                controlButtonVisibility: 'block',
                displayOrder: 1,
                displayPriority: 1,
                displayGroup: 3
        ...
        matsCollections['truth'].insert(
            {
                ...
                options: truthsModelOptionsMap[variables[0]][Object.keys(truthsModelOptionsMap[variables[0]])[0]],
                valuesMap: masterTruthValuesMap,
                superiorNames: ['variable', 'data-source'],
                controlButtonCovered: true,
                unique: false,
                default: truthsModelOptionsMap[variables[0]][Object.keys(truthsModelOptionsMap[variables[0]])[0]][0],
                controlButtonVisibility: 'block',
                displayOrder: 2,
                displayPriority: 1,
                displayGroup: 5
        ...
        if ((!matsDataUtils.areObjectsEqual(currentParam.optionsMap, truthsModelOptionsMap)) ||
            ...
            matsCollections['truth'].update({name: 'truth'}, {
                $set: {
                    ...
                    valuesMap: masterTruthValuesMap,
                    options: truthsModelOptionsMap[variables[0]][Object.keys(truthsModelOptionsMap[variables[0]])[0]],
                    default: truthsModelOptionsMap[variables[0]][Object.keys(truthsModelOptionsMap[variables[0]])[0]][1]
                }
            });
        }
    }
        ...
        matsCollections["forecast-length"].insert(
            {
                ...
                options: forecastLengthOptionsMap[variables[0]][Object.keys(forecastLengthOptionsMap[variables[0]])[0]],
                superiorNames: ['variable', 'data-source'],
                selected: '',
                controlButtonCovered: true,
                unique: false,
    ...
    const defaultDb = matsCollections["variable"].findOne({name: "variable"}, {default: 1}).default;
    modelDateRangeMap = matsCollections["variable"].findOne({name: "variable"}, {dates: 1}).dates;
...
const doCurveTextPatterns = function () {
    ...
    if (matsCollections.CurveTextPatterns.find().count() == 0) {
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region-type", "region", "statistic", "threshold", "average", "forecast-length", "valid-time", "truth", "sites", "sitesMap"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region-type", "region", "statistic", "threshold", "dieoff-type", "valid-time", "utc-cycle-start", "truth", "sites", "sitesMap", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region", "statistic", "forecast-length", "valid-time", "truth", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region-type", "region", "statistic", "threshold", "forecast-length", "truth", "sites", "sitesMap", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region-type", "region", "statistic", "threshold", "utc-cycle-start", "truth", "sites", "sitesMap"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region", "threshold", "forecast-length", "valid-time", "truth", "bin-parameter", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "variable", "data-source", "statistic", "threshold", "forecast-length", "valid-time", "truth", "sites", "sitesMap"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region", "statistic", "threshold", "forecast-length", "valid-time", "truth", "curve-dates"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
            ...
            displayParams: [
                "label", "variable", "data-source", "region", "statistic", "threshold", "forecast-length", "valid-time", "truth"
            ],
            groupSize: 6
        });
        matsCollections.CurveTextPatterns.insert({
...
Meteor.startup(function () {
    ...
    for (var didx = 0; didx < variables.length; didx++) {
        mdr.addRecord("sumPool", variableDBNames[variables[didx]].modelDB, ['threshold_descriptions']);
        mdr.addRecord("sumPool", variableDBNames[variables[didx]].sumsDB, ['regions_per_model_mats_all_categories']);
    }
    try {
