setwd("/Volumes/d1/tatiana/cvs/apps/verif/metviewer/R_work/");

source("include/Compute_STDerr.R");
source("include/util_plot.R");

strDataFile = "/Volumes/d1/tatiana/cvs/apps/verif/metviewer/R_work/data/series_stat.data";
strPlotFile = "/Users/tatiana/www/series_stat.png";

#
#  Plot Control Variables
#
boolEqualizeByIndep = FALSE;
boolEventEqual	= TRUE;
boolVertPlot	= FALSE;
boolXReverse	= FALSE;
boolShowNStats	= TRUE;
...
listPlotCI		= c("none","none","none","none");

listPlotDisp	= c(TRUE,TRUE,TRUE,TRUE);

listShowSignif  = c(FALSE,FALSE,FALSE,FALSE);

listPlotOrderSeries	= c(1,4,2,3);

dblAlpha		= 0.05;

strIndyVar		= "fcst_lead";

listIndy = c("150000", "180000", "210000", "240000");
...
listSeries1Val = list(
         `model` = c("AFWAOCv3.5.1_d01", "NoahMPv3.5.1_d01")
);
...
listLegendUsr = c("","","","Diff");

strPlotStat = "median";

listDiffSeries1=list(c("AFWAOCv3.5.1_d01 APCP_03 FAR","NoahMPv3.5.1_d01 APCP_03 FAR"));

listDiffSeries2=list();

listColors = c("#ff0000FF","#55ff00FF","#00aaffFF","#8000ffFF");
listPch = c(20,20,20,20);
listType = c("b","b","b","b");
listLty = c(1,1,1,1);
listLwd = c(1,1,1,1);
listConSeries = c(1,1,1,1);
  ...
  for( strDep1Name in names(listDep1Plot) ){
    for( strDep1Stat in listDep1Plot[[strDep1Name]] ){
      for(strSeriesVal in names(listSeries1Val)){
          ...
          fPlot = dfPlot[dfPlot$fcst_var == strDep1Name & dfPlot[[strSeriesVal]] %in% vectValPerms & dfPlot$stat_name %in% strDep1Stat,  ];
    ...
    for( strDep2Name in names(listDep2Plot) ){
      for( strDep2Stat in listDep2Plot[[strDep2Name]] ){
        for(strSeriesVal in names(listSeries2Val)){
          ...
          fPlot = dfPlot[dfPlot$fcst_var == strDep2Name & dfPlot[[strSeriesVal]] %in% vectValPerms & dfPlot$stat_name %in% strDep2Stat,  ];
...
for( listAllStatsName in names(listAllStats) ){
  ...
  listPlotData1 = buildSeriesData(listAllStats[[listAllStatsName]], strIndyVar, listIndy, strDep1Stat, listSeries1Val, listPlotDisp[indexesNumSeries1],
                                  listPlotCI[indexesNumSeries1], dblAlpha, boolVarianceInflationFactor, strPlotStat);
...
if( 0 < length(listDep2Plot) ){
  ...
  for( listAllStatsName in names(listAllStatsY2) ){
    
    listPlotData2 = buildSeriesData(listAllStatsY2[[listAllStatsName]], strIndyVar, listIndy, strDep2Stat, listSeries2Val, listPlotDisp[indexesNumSeries2],
                                    listPlotCI[indexesNumSeries2], dblAlpha, boolVarianceInflationFactor, strPlotStat);
...
if(length(listDiffSeries1) > 0){
  for( diffSeries1Name in 1: length(listDiffSeries1) ){ #1,2....
    ...
    for(var in listSeriesVar){
      dfStatsIndy1 = dfPlot[dfPlot[[var]] == diffSeriesName ,];
    }
    if(nrow(dfStatsIndy1) == 0){
      ...
      for(indy in listIndy){
        ...
        #calculate DIFF
        dfStatsIndy1$stat_value = dfStatsIndy1$stat_value - dfStatsIndy2$stat_value;
        if('stat_bcl' %in% names(dfStatsIndy1)){
            dfStatsIndy1$stat_bcl = dfStatsIndy1$stat_bcl - dfStatsIndy2$stat_bcl;
        }
        if('stat_bcu' %in% names(dfStatsIndy1)){
           dfStatsIndy1$stat_bcu = dfStatsIndy1$stat_bcu - dfStatsIndy2$stat_bcu;
        }
    ...
    listPlotData1 = buildSeriesData(plotDataDiff, strIndyVar, listIndy, strDep1Stat, listSeries1Val, listPlotDisp[indexesNumSeries1Diff],
                                    listPlotCI[indexesNumSeries1Diff], dblAlpha, boolVarianceInflationFactor, strPlotStat);
    listSeries1 = append(listSeries1, listPlotData1$series);
    if( TRUE == exists("listNStats") ){
      listNStats = listNStats + listPlotData1$nstats;
    } else {
    ...
    #create a legend
    listLegendNotOrdered[intLegendIndex] = paste("DIFF(",diffSeriesVec[1],"-",diffSeriesVec[2],")" ,collapse="");
    intLegendIndex = intLegendIndex + 1;
  }
}
...
if(length(listDiffSeries2) > 0){
  for( diffSeries2Name in 1: length(listDiffSeries2) ){ #1,2....
    ...
    listPlotData2 = buildSeriesData(plotDataDiff, strIndyVar, listIndy, strDep2Stat, listSeries2Val, listPlotDisp[indexesNumSeries2Diff],
                                    listPlotCI[indexesNumSeries2Diff], dblAlpha, boolVarianceInflationFactor, strPlotStat);
    listSeries2 = append(listSeries2, listPlotData2$series);
...
listY1Bounds = seriesMinMax(listSeries1, intNumSeries1+intNumSeries1Diff, boolLogY1);
if( TRUE == boolLogY1 ){
...
axis(
  ...
  at=listX,				          # locations of tick marks
	labels=c("15", "18", "21", "24"),	  # axis labels
...
if( TRUE == boolShowNStats ){
  axis(
    ...
    tck=.01, 				          # length of tick marks as a fraction of plot window
		...
		hadj=0.5,	  # axis label parallel location adjustment
		cex.axis=0.8, # relative axis label magnification
    col="gray"				        # axis color
  );
  ...
  mtext(
    ...
    side=intX2Axis,			    # axis of interest, 1: below, 2: left, 3: above and 4: right
		font=1,	# 1: plain text, 2: bold, 3: italic, 4: bold italic and 5: symbol
		cex=0.8,		  # relative axis label magnification
		padj=-0.5,	# axis label perpendicular location adjustment
		adj=0.5,	  # axis label parallel location adjustment
    line=1					        # margin line, starting at 0 counting outwards
  );
...
if( 0 < length(listDep2Plot) ){
  ...
  if( FALSE == boolSyncAxes ){
    ...
    listY2Bounds = seriesMinMax(listSeries2, intNumSeries2 +intNumSeries2Diff , boolLogY2);
    if( TRUE == boolLogY2 ){
  ...
  for(i in 1:(intNumSeries2 + intNumSeries2Diff)){
    ...
    lines(
      ...
      type=listTypeSeries2[i],		  # series line type - p: points, l: lines, o: overplotted points and lines,
                                    #   b or c: points joined by lines, s or S: stair steps, h: histogram, n: nothing
      pch=listPchSeries2[i],			  # point symbol - see example(points)
      font=5,				                # 1: plain text, 2: bold, 3: italic, 4: bold italic and 5: symbol
      lty=listLtySeries2[i],			  # line type - 0: blank, 1: solid, 2: dashed, 3: dotted, 4: dotdash, 5: longdash, 6: twodash
      col=listColorsSeries2[i],			# line color
      lend=1,
      cex=1,				                # line magnification
      lwd=listLwdSeries2[i]			    # line width
    );
    
    # add arrows to the line, if requested
    if( "none" != listPlotCISeries2[i] ) {
      ...
      if( 0 < sum(listDataInd) ){
        ...
        for(ind in 1: length(listXAdj)) {
          if(listShowSignifSeries2[i] == FALSE){
            lwd_current = listLwdSeries2[i];
          }else{
            if(TRUE ==boolVertPlot){
              if( listXFr[ind] < 0 && listXTo[ind] > 0){
                lwd_current = listLwdSeries2[i];
              }else{
                lwd_current = listLwdSeries2[i] + 3;
              }
    ...
    if( 0 < sum(listDataInd) ){
      ...
      for(ind in 1: length( listXAdj) ){
        if(listShowSignifSeries1[i] == FALSE){
          lwd_current = listLwdSeries1[i];
        }else{
          if(TRUE ==boolVertPlot){
            if( listXFr[ind] < 0 && listXTo[ind] > 0){
              lwd_current = listLwdSeries1[i];
            }else{
              lwd_current = listLwdSeries1[i] + 3;
            }
...
legend(
  ...
  merge=FALSE,					      # merge points and lines
	cex=.8,			  # relative magnification
	bty="o",			  # legend box type - o: box, n: none
  ...
  #agginset=c(0, -.25),			  # inset distance from margin
	inset=c(0, -.25),			# inset distance from margin
...
mtext(
	"",				  # caption
	font=1,		# 1: plain text, 2: bold, 3: italic, 4: bold italic and 5: symbol
	col="#333333",			# font color
  las=0, 							        # axis label orientation - 0: parallel to axis, 1: horiz, 2: perp to axis, 3: vert
  side=1,							        # axis of interest, 1: below, 2: left, 3: above and 4: right
	cex=0.8,			  # relative magnification
	padj=3,		# axis perpendicular location adjustment
	adj=0,			# axis parallel location adjustment
  line=1							        # margin line, starting at 0 counting outwards
);
