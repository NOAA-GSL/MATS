...
const doCurveParams = function () {
    ...
    try {
        const rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,regions,display_text,fcst_lens,scale,mindate,maxdate from regions_per_model_mats_all_categories order by display_category, display_order;");
    ...
    if (matsCollections.CurveParams.findOne({name: 'average'}) == undefined) {
        const optionsMap = {
            ...
            '90m': ['ceil(' + 3600 * 1.5 + '*floor(((m0.valid_secs)+' + 3600 * 1.5 + '/2)/' + 3600 * 1.5 + '))'],
            ...
            '12hr': ['ceil(' + 3600 * 12 + '*floor(((m0.valid_secs)+' + 3600 * 12 + '/2)/' + 3600 * 12 + '))'],
            '1D': ['ceil(' + 3600 * 24 + '*floor(((m0.valid_secs)+' + 3600 * 24 + '/2)/' + 3600 * 24 + '))'],
            '3D': ['ceil(' + 3600 * 24 * 3 + '*floor(((m0.valid_secs)+' + 3600 * 24 * 3 + '/2)/' + 3600 * 24 * 3 + '))'],
            '7D': ['ceil(' + 3600 * 24 * 7 + '*floor(((m0.valid_secs)+' + 3600 * 24 * 7 + '/2)/' + 3600 * 24 * 7 + '))'],
            '30D': ['ceil(' + 3600 * 24 * 30 + '*floor(((m0.valid_secs)+' + 3600 * 24 * 30 + '/2)/' + 3600 * 24 * 30 + '))'],
            '60D': ['ceil(' + 3600 * 24 * 60 + '*floor(((m0.valid_secs)+' + 3600 * 24 * 60 + '/2)/' + 3600 * 24 * 60 + '))'],
            '90D': ['ceil(' + 3600 * 24 * 90 + '*floor(((m0.valid_secs)+' + 3600 * 24 * 90 + '/2)/' + 3600 * 24 * 90 + '))'],
            '180D': ['ceil(' + 3600 * 24 * 180 + '*floor(((m0.valid_secs)+' + 3600 * 24 * 180 + '/2)/' + 3600 * 24 * 180 + '))'],
        };
        matsCollections.CurveParams.insert(
            {
                name: 'average',
...
Meteor.startup(function () {
    ...
    const mdr = new matsTypes.MetaDataDBRecord(matsTypes.DatabaseRoles.SUMS_DATA, "sumPool", "ptype2", ['scale_descriptions', 'ptype_descriptions', 'regions_per_model_mats_all_categories']);
    mdr.addRecord(matsTypes.DatabaseRoles.META_DATA, "metadataPool", "mats_common", ['region_descriptions']);
    try {
        matsMethods.resetApp({appMdr:mdr, appType:matsTypes.AppTypes.mats, app:'ptype', title: "Precipitation Type", group: "Precipitation"});
    } catch (error) {
