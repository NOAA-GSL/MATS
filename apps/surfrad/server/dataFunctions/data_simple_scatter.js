...
dataSimpleScatter = function (plotParams, plotFunction) {
    ...
    for (var curveIndex = 0; curveIndex < curvesLength; curveIndex++) {
        ...
        var variableX = variableOptionsMap[variableXStr];
        var variableY = variableOptionsMap[variableYStr];
        var validTimeClause = "";
        ...
        var statisticClause = "sum(" + variableX[0] + ") as square_diff_sumX, count(" + variableX[1] + ") as N_sumX, sum(" + variableX[2] + ") as obs_model_diff_sumX, sum(" + variableX[3] + ") as model_sumX, sum(" + variableX[4] + ") as obs_sumX, sum(" + variableX[5] + ") as abs_sumX, " +
            ...
            "group_concat(m0.secs, ';', " + variableX[0] + ", ';', 1, ';', " + variableX[2] + ", ';', " + variableX[3] + ", ';', " + variableX[4] + ", ';', " + variableX[5] + ", ';', " +
            variableY[0] + ", ';', 1, ';', " + variableY[2] + ", ';', " + variableY[3] + ", ';', " + variableY[4] + ", ';', " + variableY[5] + " order by m0.secs) as sub_data, count(" + variableX[0] + ") as N0";
        ...
        if (diffFrom == null) {
            ...
            if (model !== 'HRRR' && (variableXStr !== 'dswrf' && statisticXSelect !== 'Obs average')) {
                throw new Error("INFO:  The statistic/variable combination [" + statisticXSelect + " and " + variableXStr + "] is only available for the HRRR data-source.");
            } else if (model !== 'HRRR' && (variableYStr !== 'dswrf' && statisticYSelect !== 'Obs average')) {
                throw new Error("INFO:  The statistic/variable combination [" + statisticYSelect + " and " + variableYStr + "] is only available for the HRRR data-source.");
            }
