...
const doCurveParams = function () {
    ...
    if (matsCollections["statistic"].findOne({name: 'statistic'}) == undefined) {
        const optionsMap = {
            ...
            'PODy (POD of value > threshold)': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.ny)) * 100 as stat, group_concat(((m0.yy)/(m0.yy+m0.ny)) * 100, ";", m0.valid_time order by m0.valid_time) as sub_data, count(m0.yy) as N0', 'ctc', 'x100', 100],

            'PODn (POD of value < threshold)': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.yn)) * 100 as stat, group_concat(((m0.nn)/(m0.nn+m0.yn)) * 100, ";", m0.valid_time order by m0.valid_time) as sub_data, count(m0.yy) as N0', 'ctc', 'x100', 100],
            ...
            'N per graph point': ['sum(m0.yy+m0.ny+m0.yn+m0.nn+0.000) as stat, group_concat((m0.yy+m0.ny+m0.yn+m0.nn), ";", m0.valid_time order by m0.valid_time) as sub_data, count(m0.yy) as N0', 'ctc', 'Number', null]
        };
    ...
    if (matsCollections["x-axis-parameter"].findOne({name: 'x-axis-parameter'}) == undefined) {
        const optionsMap = {
            ...
            'Threshold': "select m0.thresh/100 as xVal, ",    // produces thresholds in in
    ...
    if (matsCollections["y-axis-parameter"].findOne({name: 'y-axis-parameter'}) == undefined) {
        const optionsMap = {
            ...
            'Threshold': "m0.thresh/100 as yVal, ",    // produces thresholds in in
    ...
    if (matsCollections["bin-parameter"].findOne({name: 'bin-parameter'}) == undefined) {
        const optionsMap = {
            ...
            'Init UTC hour': "select (m0.valid_time-m0.fcst_len*3600)%(24*3600)/3600 as binVal, ",
