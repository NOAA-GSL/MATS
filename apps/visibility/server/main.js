...
// this should be in the metdata someday
    ...
    try {
        const rows = matsDataUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,regions,display_text,fcst_lens,trsh from regions_per_model_mats_all_categories;");
    ...
    if (matsCollections.CurveParams.find({name: 'statistic'}).count() == 0) {
        var optionsMap = {
            ...
            'Ratio (Nlow / Ntot) * 100': ['(sum(m0.yy+m0.ny+0.000) / sum(m0.yy+m0.ny+m0.yn+m0.nn+0.000)) * 100 as stat'],
            'PODy (POD of visibility < threshold) * 100': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.ny)) * 100 as stat'],
            'PODn (POD of visibility > threshold) * 100': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.yn)) * 100 as stat'],
            'FAR (False Alarm Ratio) * 100': ['((sum(m0.yn)+0.00)/sum(m0.yn+m0.yy)) * 100 as stat'],
            ...
            'CSI (Critical Success Index) * 100': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.ny+m0.yn)) * 100 as stat'],
            'HSS (Heidke Skill Score) * 100': ['(2*(sum(m0.nn+0.00)*sum(m0.yy) - sum(m0.yn)*sum(m0.ny)) / ((sum(m0.nn+0.00)+sum(m0.ny))*(sum(m0.ny)+sum(m0.yy)) + ' +
            '(sum(m0.nn+0.00)+sum(m0.yn))*(sum(m0.yn)+sum(m0.yy)))) * 100 as  stat']
        };
