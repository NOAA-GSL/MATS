...
const doCurveParams = function () {
    ...
    try {
        const rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select scale,description from scale_descriptions;");
        var masterDescription;
        var masterScale;
        for (var j = 0; j < rows.length; j++) {
            masterDescription = rows[j].description.trim();
    ...
    try {
        const rows = matsDataQueryUtils.simplePoolQueryWrapSynchronous(sumPool, "select model,regions,display_text,fcst_lens,trsh,scale,mindate,maxdate from regions_per_model_mats_all_categories order by display_category, display_order;");
    ...
    if (matsCollections["statistic"].findOne({name: 'statistic'}) == undefined) {
        const optionsMap = {
            ...
            'PODy (POD of vil > threshold)': ['((sum(m0.yy)+0.00)/sum(m0.yy+m0.yn)) * 100 as stat, group_concat(((m0.yy)/(m0.yy+m0.yn)) * 100, ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'x100', 100],

            'PODn (POD of vil < threshold)': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.ny)) * 100 as stat, group_concat(((m0.nn)/(m0.nn+m0.ny)) * 100, ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'x100', 100],
    ...
    if (matsCollections["x-axis-parameter"].findOne({name: 'x-axis-parameter'}) == undefined) {
        const optionsMap = {
            ...
            'Threshold': "select m0.trsh as xVal, ",    // produces thresholds in kg/m2
    ...
    if (matsCollections["y-axis-parameter"].findOne({name: 'y-axis-parameter'}) == undefined) {
        const optionsMap = {
            ...
            'Threshold': "m0.trsh as yVal, ",    // produces thresholds in kg/m2
...
Meteor.startup(function () {
    ...
    mdr.addRecord("sumPool", "vil", ['threshold_descriptions', 'regions_per_model_mats_all_categories']);
    ...
    try {
        matsMethods.resetApp({appPools: allPools, appCurveParams: appCurveParams, appMdr: mdr, appType: matsTypes.AppTypes.mats, app: 'vil', title: "Vertically Integrated Liquid", group: "Radar"});
    } catch (error) {
