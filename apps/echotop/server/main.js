...
const doCurveParams = function () {
    ...
    if (matsCollections["statistic"].findOne({name: 'statistic'}) == undefined) {
        const optionsMap = {
            ...
            'PODn (POD of echo top < threshold)': ['((sum(m0.nn)+0.00)/sum(m0.nn+m0.ny)) * 100 as stat, group_concat(((m0.nn)/(m0.nn+m0.ny)) * 100, ";", m0.time order by m0.time) as sub_data, count(m0.yy) as N0', 'ctc', 'x100', 100],
    ...
    if (matsCollections["x-axis-parameter"].findOne({name: 'x-axis-parameter'}) == undefined) {
        const optionsMap = {
            ...
            'Threshold': "select m0.trsh as xVal, ",    // produces thresholds in kft
    ...
    if (matsCollections["y-axis-parameter"].findOne({name: 'y-axis-parameter'}) == undefined) {
        const optionsMap = {
            ...
            'Threshold': "m0.trsh as yVal, ",    // produces thresholds in kft
...
Meteor.startup(function () {
    ...
    mdr.addRecord("sumPool", "echotop", ['threshold_descriptions', 'regions_per_model_mats_all_categories']);
    ...
    try {
        matsMethods.resetApp({appPools: allPools, appCurveParams: appCurveParams, appMdr: mdr, appType: matsTypes.AppTypes.mats, app: 'echotop', title: "Echo Top", group: "Radar"});
    } catch (error) {
